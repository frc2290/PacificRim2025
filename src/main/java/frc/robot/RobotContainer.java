...
public class RobotContainer {
  ...
  public RobotContainer(
    ...
    auto_chooser.addOption(
        "Right1Coral", new Right1Coral(m_DiffArm, m_poseEstimator, m_state, m_manipulator));
    auto_chooser.addOption(
        "RightCoral2", new Right2Coral(m_DiffArm, m_poseEstimator, m_state, m_manipulator));
    auto_chooser.addOption(
        "RightCoral3", new Right3Coral(m_DiffArm, m_poseEstimator, m_state, m_manipulator));
    auto_chooser.addOption(
        "Left3Coral", new Left3Coral(m_DiffArm, m_poseEstimator, m_state, m_manipulator));
    auto_chooser.addOption(
        "Middle1Coral", new Middle1Coral(m_DiffArm, m_poseEstimator, m_state, m_manipulator));
    ...
    //                 m_robotDrive));
    m_robotDrive.setDefaultCommand(
        new AutomatedDrive(m_state, m_robotDrive, m_DiffArm, m_poseEstimator, m_driverController));
  }
  ...
  private void configureButtonBindings() {
    ...
    a_button
        .and(not_left_stick)
        .onTrue(m_state.setGoalCommand(PositionState.L1Position)); // Set to L1
    b_button
        .and(not_left_stick)
        .onTrue(m_state.setGoalCommand(PositionState.L2Position)); // Set to L2
    y_button
        .and(not_left_stick)
        .onTrue(m_state.setGoalCommand(PositionState.L3Position)); // Set to L3
    x_button
        .and(not_left_stick)
        .onTrue(m_state.setGoalCommand(PositionState.L4Position)); // Set to L3
    ...
    left_bumper
        .and(not_left_stick)
        .onTrue(m_state.setRightScoreCommand(false)); // Set score to left branch
    right_bumper
        .and(not_left_stick)
        .onTrue(m_state.setRightScoreCommand(true)); // Set score to right branch
    // left_bumper.and(not_left_stick).and(() -> !m_manipulator.hasCoral()).whileTrue(new
    // AlgaeRemoval(m_manipulator, m_state, false)); // Remove Algae L3
    // right_bumper.and(not_left_stick).and(() -> !m_manipulator.hasCoral()).whileTrue(new
    // AlgaeRemoval(m_manipulator, m_state, true)); // Remove Algae L2
    ...
    dpad_up
        .and(not_right_stick)
        .onTrue(m_state.setGoalCommand(PositionState.AlgaeL3Position)); // Algae L3
    dpad_down
        .and(not_right_stick)
        .onTrue(m_state.setGoalCommand(PositionState.AlgaeL2Position)); // Algae L2
    dpad_right
        .and(not_right_stick)
    ...
    // Controller Triggers
    left_trigger
        .onTrue(m_state.setDriveStateCommand(DriveState.ReefScoreMove))
        .onFalse(m_state.setDriveStateCommand(DriveState.Teleop));
    // left_trigger.and(() -> m_state.getDriveState() ==
    // DriveState.CoralStation).onTrue(m_state.setGoalCommand(PositionState.IntakePosition));
    right_trigger.onTrue(
        new ScoreCoral(m_manipulator, m_DiffArm, m_state, m_poseEstimator)); // Score coral
    ...
    hasCoral
        ...
        .or(atAlgaePos)
        .and(notAuto)
        .onFalse(m_state.setDriveStateCommand(DriveState.CoralStation))
        .onTrue(m_state.setDriveStateCommand(DriveState.Teleop));
