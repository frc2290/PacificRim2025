...
public class L2Prep extends Command{
    ...
    public L2Prep(ManipulatorStateMachine m_state, DifferentialSubsystem m_diff, ElevatorSubsystem m_elevator){
    ...
    public void initialize() {
        System.out.println("L2Prep Initializing");
        diffArm.setExtensionSetpoint(DifferentialArm.transportExtensionSetpoint);
        elevator.setElevatorSetpoint(Elevator.L2);
    ...
    public void execute(){
        ...
        if(diffArm.atRotationSetpoint() && elevator.atPosition() && diffArm.atExtenstionSetpoint()){
            atPosition = true;
            System.out.println("L2Prep Done");
        }
