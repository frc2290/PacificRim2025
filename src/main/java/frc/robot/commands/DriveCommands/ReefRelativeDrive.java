...
/** Field-relative drive mode that automatically aims the robot toward the reef center. */
public class ReefRelativeDrive extends Command {
    ...
    // PID controllers reused for reef alignment.
    private PIDController rotPid;
    private PIDController xPid;
    private PIDController yPid;

    // Cached rotation goal maintained for the alignment PID.
    private double rotTarget = 0;
    private double rotSpeed = 0;
    private Pose2d targetPose = new Pose2d();

    /**
     * Creates a driver-assist mode that keeps the robot aimed toward the reef while the driver
     * strafes into position.
     */
    public ReefRelativeDrive(DriveStateMachine m_state, DriveSubsystem m_drive, PoseEstimatorSubsystem m_poseEstimator, XboxController m_driverController) {
    ...
    public void execute(){
        ...
        if (rotPower != 0) {
            ...
            // No manual rotation requested, so aim at the reef using the pose estimator.
