...
public class AutomatedDrive extends Command {
  ...
  public void execute() {
    ...
    // Coral station state: Rotate rear of robot to face whichever coral station is closest
      ...
      rotTarget =
          poseEstimator.isClosestStationRight()
    ...
    // Reef state: Move robot to nearest branch for scoring
        ...
        || stateSubsystem.getDriveState() == DriveState.ReefScoreMove) {
    ...
    // Teleop state: If we have a coral, point robot towards center of reef
      ...
      if (stateSubsystem.hasCoral() || stateSubsystem.atAlgaePosition()) {
    ...
    // Auto state: Read target state from pose estimator that is fed from auto routine and drive to
    // it. Will hold us there too until it changes
