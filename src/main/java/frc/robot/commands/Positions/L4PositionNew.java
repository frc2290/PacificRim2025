...
public class L4PositionNew extends SequentialCommandGroup {
    private double elevatorPos = 1.725;
    private double diffExt = 25;
    private double diffRot = -165;
    private double diffExt1 = 240;

    /** Creates a new L1PositionNew. */
    public L4PositionNew(DifferentialSubsystem diffArm, ElevatorSubsystem elevator, StateSubsystem stateSubsystem) {
        if (stateSubsystem.getCurrentState() == PositionState.IntakePosition || stateSubsystem.getCurrentState() == PositionState.StartPosition) {
            ...
            Command moveRot = diffArm.setRotationSetpointCommand(diffRot);
            addCommands(rotTransport, moveExtAndElev, moveRot, stateSubsystem.setCurrentStateCommand(PositionState.L4Position));
        }
