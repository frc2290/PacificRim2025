...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  private ExponentialProfiledPIDController expPidController =
      new ExponentialProfiledPIDController(
          0.0, 0.0, 0.0, ExponentialProfile.Constraints.fromCharacteristics(0.0, 0.0, 0.0));

  // private SlewRateLimiter elevatorSlew = new SlewRateLimiter(3);

  public ElevatorSubsystem() {
    // leftMotor = new SparkFlexController(getName(), Elevator.kLeftElevatorMotorId, true, true,
    // true); //motor construct
    ...
    // rightMotor = new SparkFlexController(getName(), Elevator.kRightElevatorMotorId, true, true,
    // false); //creat second motor
  ...
  public void periodic() {
    ...
    // elevator.setReference(elevatorSlew.calculate(elevatorSetpoint), ControlType.kPosition,
    // ClosedLoopSlot.kSlot0, Elevator.kKG);
    elevatorDash.update(Constants.debugMode);
  }

  @Override
  public void simulationPeriodic() {
    if (RobotBase.isSimulation() && leftSim != null && rightSim != null && elevatorModel != null) {
      ...
      double motorOmega =
          elevatorModel.getVelocityMetersPerSecond()
              / Elevator.kSimDrumRadiusMeters
              / Elevator.kSimGearing;
      double motorAngle =
          elevatorModel.getPositionMeters() / Elevator.kSimDrumRadiusMeters / Elevator.kSimGearing;
      ...
      // Right motor follows the left and is inverted
      rightSim.iterate(-rotorRPM, busVoltage, 0.02);
      rightEncoderSim.setPosition(-Units.radiansToRotations(motorAngle));
      rightEncoderSim.setVelocity(-rotorRPM);
    }
  }
}
