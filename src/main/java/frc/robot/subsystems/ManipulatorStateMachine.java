...
/**
 * Coordinates the manipulator, elevator, and climb states via a graph of
 * commands.
 */
public class ManipulatorStateMachine extends SubsystemBase {
    ...
    /**
     * Enables interpolation based on laser distance when targeting certain reef
     * levels.
     */
     ...
     * @param climb
     */
    public ManipulatorStateMachine(DifferentialSubsystem diff, ElevatorSubsystem elevator,
            ManipulatorSubsystem manipulator, ClimbSubsystem climb) {
        m_diff = diff;
        m_elevator = elevator;
    ...
    /**
     * Builds the manipulator graph, mapping each logical state to the commands
     * required to reach it
     * safely. The graph allows the state machine to jump between presets without
     * violating joint
     * limits.
     */
    ...
    /**
     * Called by the drive state machine to indicate whether the chassis is aligned
     * to score.
     */
    ...
    /**
     * Tell state machine that command finished succesfully (Shuold only be set by
     * commands)
     * 
     * @param state
     */
    ...
    /**
     * Can only be used by commands, tell statemachine that command too to long to
     * complete, needs reset, or revert to prep node
     */
    ...
    /**
     * Check if all all subsystem are at position, atGoalState is updated by a last
     * command
     * 
     * @return
     */
    ...
    public ElevatorManipulatorState getCurrentState() {
        ...
        if (currentNode == startPositionNode)
            return ElevatorManipulatorState.START_POSITION;
        if (currentNode == safeCoralTravelNode)
            return ElevatorManipulatorState.SAFE_CORAL_TRAVEL;
        if (currentNode == intakeCoralNode)
            return ElevatorManipulatorState.INTAKE_CORAL;
        if (currentNode == l1PrepNode)
            return ElevatorManipulatorState.PrepL1;
        if (currentNode == scoreL1Node)
            return ElevatorManipulatorState.L1;
        if (currentNode == l1PostScoreNode)
            return ElevatorManipulatorState.PostL1;
        if (currentNode == l2PrepNode)
            return ElevatorManipulatorState.PrepL2;
        if (currentNode == scoreL2Node)
            return ElevatorManipulatorState.L2;
        if (currentNode == l2PostScoreNode)
            return ElevatorManipulatorState.PostL2;
        if (currentNode == l3PrepNode)
            return ElevatorManipulatorState.PrepL3;
        if (currentNode == scoreL3Node)
            return ElevatorManipulatorState.L3;
        if (currentNode == l3PostScoreNode)
            return ElevatorManipulatorState.PostL3;
        if (currentNode == l4PrepNode)
            return ElevatorManipulatorState.PrepL4;
        if (currentNode == scoreL4Node)
