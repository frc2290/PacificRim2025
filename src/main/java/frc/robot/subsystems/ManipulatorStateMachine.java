...
public class ManipulatorStateMachine extends SubsystemBase {
  ...
  public ManipulatorStateMachine(
    ...
    new Trigger(
            () ->
                getHasCoral()
                    && atDrivePose()
                    && scoreNow()
                    && !(getCurrentState() == ElevatorManipulatorState.L4))
        .onTrue(new ScoreCoral(this, manipulator));
    new Trigger(
            () ->
                getHasAlgae()
                    && scoreNow()
                    && (getCurrentState() == ElevatorManipulatorState.PROCESSOR
                        || getCurrentState() == ElevatorManipulatorState.BARGE))
        .onTrue(new ScoreAlgae(this, manipulator));
    new Trigger(
            () ->
                getHasCoral()
                    && atDrivePose()
    ...
    new Trigger(() -> scoreNow() && getCurrentState() == ElevatorManipulatorState.MANUAL)
        .onTrue(new ScoreCoral(this, manipulator));
    new Trigger(
            () ->
                scoreNow()
                    && getCurrentState() == ElevatorManipulatorState.MANUAL
                    && (elevator.getPosition() > 1))
        .onTrue(new ScoreCoral(this, manipulator));
    // new Trigger(() -> getHasCoral() && isAuto() && scoreNow() && getCurrentState() ==
    // ElevatorManipulatorState.L4).onTrue(new ScoreCoralL4(this, manipulator));
  ...
  private void initializeGraphCommand() {
    ...
    climbedNode =
        m_graphCommand.new GraphCommandNode("Climbed", new ClimberIn(m_climb), null, null);
    ...
    safeCoralTravelNode.AddNode(
        safeAlgaeTravelNode, 1.0, false); // safe travel to safe algae travel
    ...
    prepAlgaeHighNode.AddNode(
        algaeHighIntakeNode, 1.0, false); // prep algae high to algae high intake

    safeAlgaeTravelNode.AddNode(
        scoreProcessorNode, 1.0, false); // safe algae travel to score processor
    safeAlgaeTravelNode.AddNode(
        prepScoreBargeNode, 1.0, false); // safe algae travel to prep score barge
    ...
    scoreProcessorNode.AddNode(
        safeCoralTravelNode, 1.0, true); // score processor to safe coral travel
    bargePostScoreNode.AddNode(
        safeCoralTravelNode, 1.0, true); // barge post score to safe coral travel
  ...
  /**
   * Called by the drive state machine to indicate whether the chassis is aligned to score. Only set
   * by command
   */
  ...
  public Command waitUntilReady() {
    return Commands.waitUntil(() -> !isTransitioning());
  }
