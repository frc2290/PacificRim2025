...
public class DriveStateMachine extends SubsystemBase {
    ...
    private void initializeGraphCommand() {
        ...
        GraphCommandNode manualNode = m_graphCommand.new GraphCommandNode(
            "Manual",
            new PrintCommand(""),
            new PrintCommand(""),
        ...
        GraphCommandNode followPathNode = m_graphCommand.new GraphCommandNode(
            "FollowPath",
            // Autonomous-style follower that keeps path planner targets while honoring rotation overrides.
            driveCommandFactory.createFollowPathCommand(),
            new PrintCommand(""),
            new PrintCommand(""));
        nodes.put(DriveState.FOLLOW_PATH, followPathNode);

        GraphCommandNode bargeRelativeNode = m_graphCommand.new GraphCommandNode(
            "BargeRelative",
        ...
        GraphCommandNode climbRelativeNode = m_graphCommand.new GraphCommandNode(
            "ClimbRelative",
        ...
        GraphCommandNode processorRelativeNode = m_graphCommand.new GraphCommandNode(
            "ProcessorRelative",
            new PrintCommand(""),
            new PrintCommand(""),
            ...
            driveCommandFactory.createPointingAtPoseCommand(
                () -> VisionConstants.PROCESSOR_AIM_POSE,
                false));
        nodes.put(DriveState.PROCESSOR_RELATIVE, processorRelativeNode);


        GraphCommandNode coralStationNode = m_graphCommand.new GraphCommandNode(
            "CoralStation",
            new PrintCommand(""),
            new PrintCommand(""),
    ...
    private void refreshLatchedHeadings() {
        double manualRot = MathUtil.applyDeadband(driverController.getRightX(), OIConstants.kDriveDeadband);
        if (manualRot == 0.0) {
            return;
        }
