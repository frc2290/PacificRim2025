...
public class StateSubsystem extends SubsystemBase {
  ...
  public StateSubsystem(
      DifferentialSubsystem m_diff,
      ElevatorSubsystem m_elevator,
      DriveSubsystem m_drive,
      ManipulatorSubsystem m_manip,
      PoseEstimatorSubsystem m_pose,
  ...
  public boolean atAlgaePosition() {
    return (currentState == PositionState.AlgaeL2Position
        || currentState == PositionState.AlgaeL3Position);
  }
  ...
  /**
   * Cancels current running command or sequence of commands. Also sets current state to cancelled
   * and sets the subsystems to their current position
   */
  ...
  public void periodic() {
    /**
     * State management system Check if its at the goal, otherwise run the command needed to reach
     * the goal Inside each case for each state is specific controls to add moves in case it needs
     * to go somewhere else first (Mostly been moved to individual commands) Finally schedules
     * command(s) at the bottom to be executed
     */
    if (goalState != currentState
      ...
      switch (goalState) {
        ...
        case L1Position:
          ...
          //    currentCommand = currentCommand.beforeStarting(new TravelPosition(diff, elevator,
          // this));
          // }
          break;
        case L2Position:
    ...
    if (atCurrentState()) {
      ...
      } else if (getCurrentState() == PositionState.IntakePosition
  ...
  private boolean atInterpolateScoreState() {
    return currentState == PositionState.L2Position
        || currentState == PositionState.L3Position
        || currentState == PositionState.L4Position;
  }
}
