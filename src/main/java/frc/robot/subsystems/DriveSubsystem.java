...
public class DriveSubsystem extends SubsystemBase implements AutoCloseable {
  ...
  private final SimDeviceSim navxSim =
      RobotBase.isSimulation() ? new SimDeviceSim("navX-Sensor[0]") : null;
  ...
  public void updateSimState(double dt) {
    ...
    try {
      ...
      swerveDriveSim.update(forces, batteryVoltage, dt);

      if (navxYaw != null) {
        navxYaw.set(swerveDriveSim.getRotation2d().getDegrees());
      }

      m_odometry.update(swerveDriveSim.getRotation2d(), getModulePositions());
  ...
  public void simulationPeriodic() {
    // Simulation is handled in the dedicated notifier thread.
  }
