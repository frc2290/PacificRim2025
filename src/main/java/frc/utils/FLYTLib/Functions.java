...
public class Functions {
  ...
  public static double DeltaAngleRadians(double startAngle, double endAngle) {
    return ((((endAngle - startAngle - Math.PI) % (2 * Math.PI)) + (2 * Math.PI)) % (2 * Math.PI))
        - Math.PI;
  }
  ...
  public static boolean InRangeOf(double number, double referenceNumber, double rangeOf) {
    return (referenceNumber + rangeOf > number && referenceNumber - rangeOf < number);
  }

  // graph link https://www.desmos.com/calculator/ccaguak1ra
  public static double AltAxisCoord(
      double x,
      double y,
      double a) // two graph planes, on over another, you have point on the bottom one, and you have
        // know angle offset of the top one relative tot eh bottom one, function will return
        // coordinates of the top one
      {
