...
/**
 * Simulation model for a single swerve module. This class is responsible for integrating the steer
 * motor state and computing the longitudinal wheel force generated by the drive motor based on the
 * current rolling speed and applied voltage.
 *
 * <p>The intent of this class is to be lightweight and independent of any particular robot
 * implementation. Sensor mirroring is performed by writing to the provided Spark simulation encoder
 * objects when available, but these are optional and may be {@code null} for users that only care
 * about the physics results.
 *
 * <p>The steering motor is expected to be modeled with a NEO 550 and its corresponding gear
 * reduction (e.g. 46.45:1 on a REV MAXSwerve module) so that the internal closed-loop controller
...
public class SwerveModuleSim {
  ...
  /**
   * Record containing the longitudinal force vector produced by the wheel and the corresponding
   * rolling speed used to generate it.
   */
   ...
   * @param steerSim optional SparkMaxSim for the steering motor. When provided, the sim's internal
   *     closed-loop algorithms are used to update the module azimuth so steer volts need not be
   *     supplied externally.
  ...
  public SwerveModuleSim(
    ...
    m_steerModel =
        steerMotor != null
            ? new DCMotorSim(
                LinearSystemId.createDCMotorSystem(steerMotor, 0.01, m_steerGearRatio), steerMotor)
            : null;
  ...
  /**
   * Performs a simulation step for this module. The steering angle is updated and the drive force
   * is calculated based on the robot's previous body-frame velocity.
   *
  ...
  /**
   * Sets the applied output of the simulated drive motor. This allows open-loop voltage control
   * during tests without requiring a {@link SparkClosedLoopController}.
   *
  ...
  public void updateDriveSensor(double vRoll, double dt, double busVoltage) {
    ...
    if (m_driveEncoderSim != null) {
      double newPos =
          m_driveEncoderSim.getPosition() + (vRoll * dt) / distPerRot; // motor rotations
